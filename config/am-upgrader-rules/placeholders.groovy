...
def getRules() {
    return [
            forRealm("/",
                    replace("aliases")
                            .with(objectProvider(singletonMap("\$list", "&{am.server.hostnames|&{am.server.fqdn},&{fqdn},am,am-config}")))),
            ...
            forGlobalService("CorsService",
                forDefaultInstanceSettings(
                            forNamedInstanceSettings("cors",
                                    replace("acceptedOrigins")
                                        .with([ "&{lb.protocol|https}://&{fqdn}" ])))),
